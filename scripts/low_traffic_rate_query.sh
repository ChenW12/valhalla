./../build/valhalla_service ../valhalla.json route '{"locations":[{"lat":51.50405,"lon":-0.19222},{"lat":51.5092,"lon":-0.1954}],"costing":"pedestrian","costing_options":{"pedestrian":{"use_lit":0,"less_crime_rate":false,"low_traffic_rate":true}},"directions_options":{"units":"kilometers"},"date_time":{"type":1,"value":"2024-05-06T18:00"}}' | jq '.'

# Used when set up the server
curl http://localhost:8002/route -d '{"locations":[{"lat":51.50405,"lon":-0.19222},{"lat":51.5092,"lon":-0.1954}],"costing":"pedestrian","costing_options":{"pedestrian":{"use_lit":0,"less_crime_rate":false,"low_traffic_rate":true}},"directions_options":{"units":"kilometers"},"date_time":{"type":1,"value":"2024-05-06T18:00"}}' | jq '.'

# Passing Kensington Church Street
# 51.50405,-0.19222
# 51.5092,-0.1954
